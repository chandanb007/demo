<template>
  <div id="app">
    <form @submit.prevent="validateBeforeSubmit" v-if="!formSubmitted">
      ...
      <h1>Add Primary city & Liecense</h1>
      <div
        class="form-group"
      ...
      >
        <label class="control-label" for="email">
          <input
            ...
            name="primary_city"
            :value="city.id"
            v-model="primary_city"
          />
          {{ city.name }}
        </label>
        <input
          v-model="city.liecense"
          class="form-control"
          type="text"
          :placeholder="`Enter liecens id for ` + city.name"
        />
        <p class="text-danger" v-if="errors.has('email')">
          {{ errors.first("email") }}
        </p>
...
export default {
  ...
  watch: {
    ...
    primary_city(newQuestion, oldQuestion) {
      this.cities.forEach((element) => {
        if (element.id === newQuestion) {
          this.primary_liecense = element.liecense ? element.liecense : "";
        }
      });
    },
  },
  methods: {
    ...
    submitForm() {
      ...
      this.cities.forEach((element) => {
        if (element.id === this.primary_city) {
          this.primary_liecense = element.liecense ? element.liecense : "";
        }
      });
      axios
        .post("http://127.0.0.1:8000/api/postdata", {
          primary_city: this.primary_city,
          primary_liecense: this.primary_liecense,
          lic: this.cities,
          id: this.id,
        })
    ...
    async getData() {
      ...
      console.log(response);
      this.cities = JSON.parse(response.data.other_liecense);
      this.primary_city = response.data.city_id;
      ...
      //   .then(function (response) {
      //     //this.cities = JSON.parse(response.data.other_liecense);
      //     console.log(JSON.parse(response.data.other_liecense));
      //   })
      //   .catch(function (error) {
      //     console.log(error);
      //   });
